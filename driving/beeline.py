...
from base.action import Action, DriveAction
...
class BeeLine(Action):
    def __init__(self, car:Car, target:vec3):
        super().__init__(car)
        self.target = target
        self.drive = DriveAction(self.car, target)
    ...
    def can_interrupt(self):
        if self.action is self.drive and self.car.on_ground:
            return True
        else:
            return self.action.can_interrupt
    ...
    def step(self, dt:float):
        ...
        if self.stage == 2:
            if self.flip.finished:
                self.drive = DriveAction(self.car, self.target)
                self.action = self.drive
                next_stage = 0
