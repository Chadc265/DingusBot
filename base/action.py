...
from util.constants import MAX_DRIVING_SPEED, GRAVITY, BOOST_ACCELERATION, M
class Action:
...
class DriveAction(Drive):
    ...
    def set_speed_for_arrival(self, dt:float):
    ...
    def simulate(self, sim_target:vec3):
        car = Car(self.car)
        action = DriveAction(car, sim_target)
        dt = 0.016666
        for i in range(5.0/dt):
            action.step(dt)
            car.step(dt)
            if action.finished:
                break
